...
`ifdef FI_ASSERT_READY_BUSY_TIMEOUT
    reg [7:0] ready_wait_cnt;
`endif
    ...
    always @(*) begin
        ...
        case (state)
            IDLE: begin
`ifdef FI_ASSERT_READY_BUSY_TIMEOUT
                if (ready && ready_wait_cnt == 8'd120) next_state = L0_FETCH;
`else
...
`endif
    ...
    always @(posedge clk or posedge reset) begin
        if (reset) begin
            busy <= 0;
            cwr <= 0;
`ifdef FI_ASSERT_RESET_PROTOCOL
            cwr <= 1;
`endif
            crd <= 0;
            iaddr <= 0;
...
`ifdef FI_ASSERT_READY_BUSY_TIMEOUT
                    if (!ready) begin
                        ready_wait_cnt <= 0;
                    end else if (ready_wait_cnt != 8'd120) begin
                        ready_wait_cnt <= ready_wait_cnt + 1'b1;
                    end else begin
                        busy <= 1;
                        pixel_cnt <= 0;
                        filter_cnt <= 0;
...
`endif
                ...
                L0_WRITE: begin
`ifdef FI_BUG3_MISSING_L0
                    cwr <= (pixel_cnt == 12'd123) ? 1'b0 : 1'b1;
`else
                    cwr <= 1;
`endif
`ifdef FI_ASSERT_ILLEGAL_CSEL
                    csel <= (pixel_cnt == 12'd123) ? 3'b010 : 3'b001;
`else
                    csel <= 3'b001;
`endif
                    caddr_wr <= pixel_cnt;
`ifdef FI_BUG1_L0_DATA
                    cdata_wr <= (pixel_cnt == 12'd123) ? (relu_out + 20'd1) : relu_out;
`else
                    cdata_wr <= relu_out;
`endif
                ...
                L1_WRITE: begin
                    cwr <= 1;
                    csel <= 3'b011;
`ifdef FI_ASSERT_L1_ADDR_OOB
                    caddr_wr <= (pixel_cnt == 12'd17) ? 12'd1024 : pixel_cnt;
`elsif FI_BUG4_L1_DUP_ADDR
                    caddr_wr <= (pixel_cnt == 12'd1) ? 12'd0 : pixel_cnt;
`else
                    caddr_wr <= pixel_cnt;
`endif
`ifdef FI_BUG2_L1_DATA
                    cdata_wr <= (pixel_cnt == 12'd17) ? (max_val + 20'd1) : max_val;
`else
                    cdata_wr <= max_val;
`endif
